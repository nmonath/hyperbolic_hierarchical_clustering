...
import cc.factorie.util.{DefaultCmdOptions, JavaHashSet, Threading}
...
class EvalTreeNode(
                    ...
                    var children: ArrayBuffer[EvalTreeNode], var labels: ArrayBuffer[String]) extends Comparable[EvalTreeNode]{


  override def compareTo(o: EvalTreeNode): Int = id.compareTo(o.id)

  var best = (0,0)
  var local = (0,0)

  lazy val labelCounts = labels.groupBy(identity).mapValues(_.size).toMap[String, Int]

  var bestClusteringRoots = ArrayBuffer[EvalTreeNode](this)
  var bestClustering = new ArrayBuffer[Iterable[EvalTreeNode]]()
  /**
  ...
  var leafCache: ArrayBuffer[EvalTreeNode] = null
  /**
  ...
  def purity(wrt: String): Double = {
    ...
    labelCounts(wrt).toDouble / labels.size
  }

  /**
    * Some leaves may have missing labels. Purity strict
    * differs from purity by penalizing the inclusion of these
    * leaves with missing labels in the cluster.
    * @param wrt Class label about which purity should be computed
    * @return Purity value for the given class
    */
  def purityStrict(wrt: String): Double = {
    // TODO: We could cache these scores
    labels.count(_ == wrt).toDouble / this.leaves().size
  }
...
//          println(s"Read $idx lines of $fn")
    ...
    leaves.filter(_.labels.nonEmpty).foreach(propagateLabel)
    nodes.values.zipWithIndex.foreach{
        case (n,idx) =>
            n.leaves()
            if (idx % 10000 == 0)
                System.err.print(s"\r[eval dp] pre-processed ${idx} nodes")
    }
    System.err.println()
...
trait DendrogramPurityOpts extends DefaultCmdOptions {
  ...
  val strictDP = new CmdOption[Boolean]("strict",false,"BOOLEAN","Whether to use version of purity which penalizes missing labels. ")
}
...
object DendrogramPurity {
  ...
  def allPairsForEval(root: EvalTreeNode, numThreads: Int): ArrayBuffer[(EvalTreeNode,EvalTreeNode)] = {
    ...
    leaves.filter(_.labels.nonEmpty).zipWithIndex.foreach{
      case (l,idx) =>
//        if (idx % 100 == 0)
...
//          byClass(l.labels.head) += l
        // only include laves that have a label.
        if (l.labels.size == 1) {
          byClass.getOrElseUpdate(l.labels.head, new ArrayBuffer[EvalTreeNode]()) += l
        }
    }
//    println("Finding pairs.")
  ...
  /**
    ...
    */
  def evalPar(pairs: IndexedSeq[(EvalTreeNode,EvalTreeNode)],threads:Int,print:Boolean, strict: Boolean =false): Double = {
    ...
    Threading.parForeach(startIndexes,threads)({
      start =>
        ...
        while (i < end) {
          val pair = pairs(i)
          if (strict)
            local_purity += pair._1.lca(pair._2).purityStrict(pair._1.labels.head)
          else
...
object ExpectedDendrogramPurity {
  ...
  def all_pairs_for_eval(root: EvalTreeNode, idSet: Set[String],numThreads: Int): ArrayBuffer[(EvalTreeNode,EvalTreeNode)] = {
    val leaves = root.leaves()
    val byClass = new util.HashMap[String,ArrayBuffer[EvalTreeNode]]().asScala
    leaves.filter(l => idSet.contains(l.id)).zipWithIndex.foreach{
      case (l,idx) =>
//        if (idx % 100 == 0)
//          println(s"Processed $idx leaves")
        // if labels are empty do not include in evaluation.
        if (l.labels.size == 1) {
          byClass.getOrElseUpdate(l.labels.head, new ArrayBuffer[EvalTreeNode]()) += l
        }
    }
//    println("Finding pairs.")
  ...
  /**
    ...
    * @return
    */
  def evalPar(pairs: IndexedSeq[(EvalTreeNode,EvalTreeNode)],threads:Int,print:Boolean,strict: Boolean = false) = {
    val bufferSize = 1000
    val startIndexes = 0 until pairs.size by bufferSize
    @volatile var sum_purities = 0.0
    @volatile var N = 0.0
    Threading.parForeach(startIndexes,threads)({
      start =>
        ...
        while (i < end) {
          val pair = pairs(i)
          // if the code below breaks you tried to evaluate on a pair that has no label.
          assert(pair._1.labels.size == 1)
          assert(pair._2.labels.size == 1)
          if (strict)
            local_purity += pair._1.lca(pair._2).purityStrict(pair._1.labels.head)
          else
            local_purity += pair._1.lca(pair._2).purity(pair._1.labels.head)
